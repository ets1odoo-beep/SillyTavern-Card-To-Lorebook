...
================================================================================
OMISSION POLICY - read carefully
================================================================================

ANCHOR IS ALWAYS REQUIRED FOR SUBSTANTIVE CARD TEXT - never omit:
The primaryCharacter's anchor entry MUST be emitted when the card includes real descriptive text. The anchor is the identity record - name + species/race + gender + role + one distinctive identifier. Use explicit facts from the card. If the source text gives species/race, gender, age, occupation, role, or identifiers, preserve them exactly. If the input contains only name/tags, do not invent missing identity facts from tags.
...
SOURCE FIDELITY POLICY for anchor / appearance / personality / voice:
- Extract these sections from actual card fields: description, personality, scenario, first message, example dialogue, creator notes, system prompt, post-history instructions, alternate greetings, and embedded lore.
- Preserve explicit facts exactly: race/species, gender, age, height, body details, clothing, speech, history, rules, kinks, taboos, and scene constraints.
- Do NOT invent race, gender, appearance, personality, backstory, clothing, kinks, or relationships from tags alone.
- Tags may only help classify or key facts that are already supported by substantive card text.
- DO NOT write meta-disclaimers like "appearance not established", "personality to be defined through play", "voice details unclear". If source text is missing, omit that non-anchor section. The extension will retry or flag weak output.
- For OTHER entities (locations, items, factions, etc.): only emit when the card actually mentions the entity. Don't invent locations / factions / quests the card never references.

MINIMUM OUTPUT FOR EVERY SUBSTANTIVE CARD: at least primaryCharacter with at least anchor + appearance + personality entries when the card provides real descriptive text. If the input contains only name/tags, do not invent; return the best supported anchor only.
...
const DEFAULT_PROFILES = [
 ...
 // entities) so we need generous output budget. Thinking models
 // (GLM-5.1:thinking, DeepSeek-V4, Claude-Opus-thinking, etc.) burn
 // several thousand tokens on reasoning before writing the JSON - 8000
 // was tight, 16000 gives headroom so the output isn't truncated to
 // empty after the reasoning trace finishes.
...
export function settings() {
 ...
 //
 // Always refreshed: name, baseInstruction, builtin marker.
 // Bumped only-up: responseLength (never decrease - user may have tuned
 // higher; but if saved is below current default, promote it so thinking
 // models don't truncate output).
 // Preserved: includeFields, wordCap, outputFormat, aiConnectionProfile,
 // conflictPolicy, maxEstimatedTokens, generateRoster,
 // alsoExtractCardSummary, entrySplitMode, sectionOverrides.
 //
 // Cloned profiles (builtin === false) are never touched.
 ...
 // Promote responseLength up to current default if saved is lower -
 // never decrease (user may have explicitly bumped higher).
 const savedRL = Number(saved.responseLength) || 0;
 const defRL = Number(defaults.responseLength) || 0;
 if (defRL > 0 && savedRL < defRL) {
 saved.responseLength = defRL;
 }
 }
